...
export function getInputClass(that, isSelect = false) {
  const formControlClass = isSelect
    ? that.constants.classes.select
    : that.constants.classes.input;

  return that.options.iconSize
...
export function addOptionToSelectControl(
  selectControl,
  _value,
  text,
  selected,
  ...
  let value =
    _value === undefined || _value === null ? "" : _value.toString().trim();
  ...
  const isSelected = shouldCompareText
...
export function setValues(that) {
  ...
  if (that._valuesFilterControl.length > 0) {
    ...
    searchControls.each((i, el) => {
      ...
      result = that._valuesFilterControl.filter(
        (valueObj) => valueObj.field === field
      );
            ...
            const closedCallback = () => {
              ...
              if (Array.isArray(cacheElementInfo.value)) {
                ...
                $.each(cacheElementInfo.value, function (i, e) {
                  $element
                    .find(Utils.sprintf("option[value='%s']", e))
                    .prop("selected", true);
                });
              } else {
...
export function collectBootstrapTableFilterCookies() {
  const cookies = [];
  const foundCookies = document.cookie.match(
    /bs\.table\.(filterControl|searchText)/g
  );
...
export function isColumnSearchableViaSelect({ filterControl, searchable }) {
  return (
    filterControl && filterControl.toLowerCase() === "select" && searchable
  );
}
...
export function initFilterSelectControls(that) {
  ...
  $.each(that.header.fields, (j, field) => {
    ...
    const selectControl = getControlContainer(that).find(
      `select.bootstrap-table-filter-control-${escapeID(column.field)}`
    );
    ...
    if (
      ...
      if (
        !selectControl[0].multiple &&
        selectControl.get(selectControl.length - 1).options.length === 0
      ) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        const formatter =
          that.options.editable && column.editable
            ? column._formatter
            : that.header.formatters[j];
        let formattedValue = Utils.calculateObjectValue(
          that.header,
          formatter,
          [fieldValue, data[i], i],
          fieldValue
        ...
        if (column.filterDataCollector) {
          formattedValue = Utils.calculateObjectValue(
            that.header,
            column.filterDataCollector,
            [fieldValue, data[i], formattedValue],
            formattedValue
          );
        }
...
export function createControls(that, header) {
  ...
  $.each(that.columns, (_, column) => {
    ...
    if (!column.filterControl && !that.options.filterControlContainer) {
      ...
      const $filterControls = $(
        `.bootstrap-table-filter-control-${column.field}`
      );
    ...
    if (
      ...
      "" !== column.filterDefault &&
      "undefined" !== typeof column.filterDefault
    ) {
    ...
    if (column.filterData && column.filterData.toLowerCase() !== "column") {
      ...
      if (filterDataType) {
        ...
        selectControl = header.find(
          `.bootstrap-table-filter-control-${escapeID(column.field)}`
        );
  ...
  if (addedFilterControl) {
    header
      .off("keyup", "input")
      .on("keyup", "input", ({ currentTarget, keyCode }, obj) => {
    ...
    header
      .off("change", "select", ".fc-multipleselect")
      .on(
        "change",
        "select",
        ".fc-multipleselect",
    ...
    header
      .off("mouseup", "input:not([type=radio])")
      .on(
        "mouseup",
    ...
    header
      ...
      .on("change", "input[type=radio]", ({ currentTarget, keyCode }) => {
        clearTimeout(currentTarget.timeoutId || 0);
        currentTarget.timeoutId = setTimeout(() => {
          that.onColumnSearch({ currentTarget, keyCode });
        }, that.options.searchTimeOut);
    ...
    if (header.find(".date-filter-control").length > 0) {
      $.each(
        ...
        (
          i,
          { filterDefault, filterControl, field, filterDatepickerOptions }
        ) => {
          if (
            filterControl !== undefined &&
            filterControl.toLowerCase() === "datepicker"
          ) {
            const $datepicker = header.find(
...
export function getDirectionOfSelectOptions(_alignment) {
  const alignment =
    _alignment === undefined ? "left" : _alignment.toLowerCase();
